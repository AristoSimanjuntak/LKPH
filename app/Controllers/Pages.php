<?php

namespace App\Controllers;

class Pages extends BaseController
{

	public function index()
	{
		return view('pages/index');
	}
	public function login()
	{
		return view('pages/login');
	}

	public function crypto_dashbaord()
	{
		return view('pages/crypto-dashbaord');
	}

	public function crypto_market()
	{
		return view('pages/crypto-market');
	}

	public function crypto_currency_exchange()
	{
		return view('pages/crypto-currency-exchange');
	}

	public function crypto_buy_sell()
	{
		return view('pages/crypto-buy-sell');
	}

	public function crypto_wallet()
	{
		return view('pages/crypto-wallet');
	}
	public function crypto_transcations()
	{
		return view('pages/crypto-transcations');
	}

	public function ecommerce_dashboard()
	{
		return view('pages/ecommerce-dashboard');
	}

	public function ecommerce_products()
	{
		return view('pages/ecommerce-products');
	}

	public function ecommerce_product_details()
	{
		return view('pages/ecommerce-product-details');
	}

	public function ecommerce_cart()
	{
		return view('pages/ecommerce-cart');
	}

	public function ecommerce_orders()
	{
		return view('pages/ecommerce-orders');
	}

	public function ecommerce_checkout()
	{
		return view('pages/ecommerce-checkout');
	}

	public function ecommerce_account()
	{
		return view('pages/ecommerce-account');
	}

	public function widgets()
	{
		return view('pages/widgets');
	}

	public function mail_inbox()
	{
		return view('pages/mail-inbox');
	}

	public function viewmail()
	{
		return view('pages/viewmail');
	}

	public function chat()
	{
		return view('pages/chat');
	}

	public function cards()
	{
		return view('pages/cards');
	}

	public function calendar()
	{
		return view('pages/calendar');
	}

	public function contacts()
	{
		return view('pages/contacts');
	}

	public function icons1()
	{
		return view('pages/icons1');
	}

	public function icons2()
	{
		return view('pages/icons2');
	}

	public function icons3()
	{
		return view('pages/icons3');
	}

	public function icons4()
	{
		return view('pages/icons4');
	}

	public function icons5()
	{
		return view('pages/icons5');
	}

	public function icons6()
	{
		return view('pages/icons6');
	}

	public function icons7()
	{
		return view('pages/icons7');
	}

	public function icons8()
	{
		return view('pages/icons8');
	}

	public function icons9()
	{
		return view('pages/icons9');
	}

	public function icons10()
	{
		return view('pages/icons10');
	}

	public function icons11()
	{
		return view('pages/icons11');
	}

	public function map_mapel()
	{
		return view('pages/map-mapel');
	}

	public function map_vector()
	{
		return view('pages/map-vector');
	}

	public function table_basic()
	{
		return view('pages/table-basic');
	}

	public function table_data()
	{
		return view('pages/table-data');
	}
	public function alerts()
	{
		return view('pages/alerts');
	}

	public function avatar()
	{
		return view('pages/avatar');
	}

	public function badge()
	{
		return view('pages/badge');
	}

	public function thumbnails()
	{
		return view('pages/thumbnails');
	}

	public function table_badge()
	{
		return view('pages/badge');
	}

	public function breadcrumbs()
	{
		return view('pages/breadcrumbs');
	}

	public function buttons()
	{
		return view('pages/buttons');
	}

	public function dropdown()
	{
		return view('pages/dropdown');
	}

	public function list_group()
	{
		return view('pages/list-group');
	}

	public function navigation()
	{
		return view('pages/navigation');
	}

	public function pagination()
	{
		return view('pages/pagination');
	}

	public function popover()
	{
		return view('pages/popover');
	}

	public function progress()
	{
		return view('pages/progress');
	}

	public function spinners()
	{
		return view('pages/spinners');
	}

	public function media_object()
	{
		return view('pages/media-object');
	}

	public function typography()
	{
		return view('pages/typography');
	}

	public function tooltip()
	{
		return view('pages/tooltip');
	}

	public function toast()
	{
		return view('pages/toast');
	}

	public function tags()
	{
		return view('pages/tags');
	}

	public function accordion()
	{
		return view('pages/accordion');
	}

	public function carousel()
	{
		return view('pages/carousel');
	}

	public function collapse()
	{
		return view('pages/collapse');
	}

	public function modals()
	{
		return view('pages/modals');
	}

	public function timeline()
	{
		return view('pages/timeline');
	}

	public function darggablecard()
	{
		return view('pages/darggablecard');
	}

	public function sweet_alert()
	{
		return view('pages/sweet-alert');
	}

	public function rating()
	{
		return view('pages/rating');
	}

	public function search()
	{
		return view('pages/search');
	}

	public function userlist()
	{
		return view('pages/userlist');
	}

	public function blog()
	{
		return view('pages/blog');
	}

	public function form_elements()
	{
		return view('pages/form-elements');
	}

	public function form_advanced()
	{
		return view('pages/form-advanced');
	}

	public function form_layouts()
	{
		return view('pages/form-layouts');
	}

	public function form_validation()
	{
		return view('pages/form-validation');
	}

	public function form_wizards()
	{
		return view('pages/form-wizards');
	}

	public function form_editor()
	{
		return view('pages/form-editor');
	}

	public function chart_chartjs()
	{
		return view('pages/chart-chartjs');
	}

	public function chart_echart()
	{
		return view('pages/chart-echart');
	}

	public function chart_flot()
	{
		return view('pages/chart-flot');
	}

	public function chart_morris()
	{
		return view('pages/chart-morris');
	}

	public function chart_spark_peity()
	{
		return view('pages/chart-spark-peity');
	}

	public function profile()
	{
		return view('pages/profile');
	}

	public function invoice()
	{
		return view('pages/invoice');
	}

	public function pricing()
	{
		return view('pages/pricing');
	}

	public function gallery()
	{
		return view('pages/gallery');
	}

	public function faq()
	{
		return view('pages/faq');
	}

	public function success_message()
	{
		return view('pages/success-message');
	}

	public function danger_message()
	{
		return view('pages/danger-message');
	}

	public function warning_message()
	{
		return view('pages/warning-message');
	}

	public function empty()
	{
		return view('pages/empty');
	}

	public function background()
	{
		return view('pages/background');
	}

	public function border()
	{
		return view('pages/border');
	}

	public function display()
	{
		return view('pages/display');
	}

	public function flex()
	{
		return view('pages/flex');
	}

	public function height()
	{
		return view('pages/height');
	}

	public function margin()
	{
		return view('pages/margin');
	}

	public function padding()
	{
		return view('pages/padding');
	}

	public function position()
	{
		return view('pages/position');
	}

	public function extras()
	{
		return view('pages/extras');
	}

	public function signin()
	{
		return view('pages/signin');
	}

	public function signup()
	{
		return view('pages/signup');
	}

	public function forgot()
	{
		return view('pages/forgot');
	}

	public function reset()
	{
		return view('pages/reset');
	}

	public function lockscreen()
	{
		return view('pages/lockscreen');
	}

	public function underconstruction()
	{
		return view('pages/underconstruction');
	}

	public function error_404()
	{
		return view('pages/error-404');
	}

	public function error_500()
	{
		return view('pages/error-500');
	}

	public function width()
	{
		return view('pages/width');
	}

	public function horizontallight()
	{
		return view('pages/horizontallight');
	}

	public function horizontal_light_boxed()
	{
		return view('pages/horizontal-light-boxed');
	}

	public function horizontaldark()
	{
		return view('pages/horizontaldark');
	}

	public function horizontal_dark_boxed()
	{
		return view('pages/horizontal-dark-boxed');
	}

	public function horizontal_light_center_logo_boxed()
	{
		return view('pages/horizontal-light-center-logo-boxed');
	}

	public function horizontal_dark_centerlogo()
	{
		return view('pages/horizontal-dark-centerlogo');
	}

	public function horizontal_light_centerlogo()
	{
		return view('pages/horizontal-light-centerlogo');
	}

	public function horizontal_dark_center_logo_boxed()
	{
		return view('pages/horizontal-dark-center-logo-boxed');
	}

	public function sidemenu_light()
	{
		return view('pages/sidemenu-light');
	}

	public function sidemenu_light_boxed()
	{
		return view('pages/sidemenu-light-boxed');
	}

	public function sidemenu_dark()
	{
		return view('pages/sidemenu-dark');
	}

	public function sidemenu_dark_boxed()
	{
		return view('pages/sidemenu-dark-boxed');
	}

	public function sidemenu_light_closed()
	{
		return view('pages/sidemenu-light-closed');
	}

	public function sidemenu_light_closed_boxed()
	{
		return view('pages/sidemenu-light-closed-boxed');
	}

	public function sidemenu_dark_closed()
	{
		return view('pages/sidemenu-dark-closed');
	}

	public function sidemenu_dark_closed_boxed()
	{
		return view('pages/sidemenu-dark-closed-boxed');
	}

	public function sidemenu_light_icon_overlay()
	{
		return view('pages/sidemenu-light-icon-overlay');
	}

	public function sidemenu_light_icon_overlay_dark()
	{
		return view('pages/sidemenu-light-icon-overlay-dark');
	}

	public function sidemenu_light_icon_overlay_boxed()
	{
		return view('pages/sidemenu-light-icon-overlay-boxed');
	}

	public function sidemenu_light_icon_overlay_dark_boxed()
	{
		return view('pages/sidemenu-light-icon-overlay-dark-boxed');
	}

	public function sidemenu_toggle_light_sidebar()
	{
		return view('pages/sidemenu-toggle-light-sidebar');
	}

	public function sidemenu_toggle_light_sidebar_dark()
	{
		return view('pages/sidemenu-toggle-light-sidebar-dark');
	}

	public function sidemenu_toggle_light_sidebar_boxed()
	{
		return view('pages/sidemenu-toggle-light-sidebar-boxed');
	}

	public function sidemenu_toggle_light_sidebar_dark_boxed()
	{
		return view('pages/sidemenu-toggle-light-sidebar-dark-boxed');
	}

	public function sidemenu_icontext()
	{
		return view('pages/sidemenu-icontext');
	}

	public function sidemenu_icontext_dark()
	{
		return view('pages/sidemenu-icontext-dark');
	}

	public function sidemenu_icontext_boxed()
	{
		return view('pages/sidemenu-icontext-boxed');
	}

	public function sidemenu_icontext_dark_boxed()
	{
		return view('pages/sidemenu-icontext-dark-boxed');
	}

	public function sidemenu_hoversubmenu()
	{
		return view('pages/sidemenu-hoversubmenu');
	}

	public function sidemenu_hoversubmenu_dark()
	{
		return view('pages/sidemenu-hoversubmenu-dark');
	}

	public function sidemenu_hoversubmenu_boxed()
	{
		return view('pages/sidemenu-hoversubmenu-boxed');
	}

	public function sidemenu_hoversubmenu_dark_boxed()
	{
		return view('pages/sidemenu-hoversubmenu-dark-boxed');
	}

	public function sidemenu_hoversubmenu_style_1()
	{
		return view('pages/sidemenu-hoversubmenu-style-1');
	}

	public function sidemenu_hoversubmenu_style_1_dark()
	{
		return view('pages/sidemenu-hoversubmenu-style-1-dark');
	}

	public function sidemenu_hoversubmenu_style_1_boxed()
	{
		return view('pages/sidemenu-hoversubmenu-style-1-boxed');
	}

	public function sidemenu_hoversubmenu_style_1_dark_boxed()
	{
		return view('pages/sidemenu-hoversubmenu-style-1-dark-boxed');
	}
}
